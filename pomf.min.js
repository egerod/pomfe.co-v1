document.addEventListener("DOMContentLoaded", function () {
	...
	function showUploadResult(a) {
		...
		var status = target.status, //f
			span = document.createElement("span");
		span.className = "file-url"
		row.appendChild(span);
		var aElement = document.createElement("a");
		if (200 === status) {
			var i = JSON.parse(target.responseText);
			i.success ? (aElement.textContent = i.files[0].url.replace(/.*?:\/\//g, ""), aElement.href = i.files[0].url, span.appendChild(aElement)) : bar.innerHTML = "Error: " + i.description
